...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (savedOrders) {
      try {
        ...
        console.error("فشل تحميل الطلبات السابقة");
      }
    }
  }, []);
  ...
  return (
    <div className="min-h-screen pb-20 bg-gray-50 text-gray-800">
            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {MENU_ITEMS.map(drink => (
                  <div key={drink.id} className="bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-md transition-shadow border border-gray-100">
                    ...
                    <div className="p-4 flex justify-between items-center">
                      ...
                      <button 
                        ...
                        className={`px-6 py-2 rounded-xl font-bold transition-all ${selectedClinic ? 'bg-emerald-500 text-white hover:bg-emerald-600 active:scale-95' : 'bg-gray-200 text-gray-400 cursor-not-allowed'}`}
                      >
                        اطلب
                      </button>
                    </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              <div className="space-y-4">
                ...
                {orders.filter(o => o.status === 'pending').length === 0 ? (
                  <div className="bg-white p-12 text-center rounded-2xl border-2 border-dashed border-gray-200 text-gray-400">لا توجد طلبات معلقة</div>
                ) : (
                  orders.filter(o => o.status === 'pending').map(order => (
                    <div key={order.id} className="bg-white p-4 rounded-xl shadow-sm border-r-4 border-orange-500 flex flex-col gap-3">
                      <div className="flex justify-between items-start">
                        ...
                        <div className="flex gap-2">
                          <button onClick={() => updateOrderStatus(order.id, 'completed')} className="bg-emerald-500 text-white p-2 rounded-lg hover:bg-emerald-600"><Icons.Check /></button>
                          <button onClick={() => updateOrderStatus(order.id, 'cancelled')} className="bg-red-50 text-red-500 p-2 rounded-lg hover:bg-red-100 text-xs font-bold">إلغاء</button>
                        </div>
                      </div>
                      {order.notes && (
                        <div className="bg-orange-50 p-3 rounded-lg border border-orange-100 text-sm text-orange-800 italic flex items-center gap-2">
              ...
              <div className="space-y-4">
                <h3 className="font-bold text-lg text-gray-500">السجل</h3>
                <div className="space-y-2">
                  {orders.filter(o => o.status !== 'pending').sort((a,b) => b.timestamp - a.timestamp).slice(0, 10).map(order => (
                    <div key={order.id} className={`p-3 rounded-lg border text-sm flex justify-between items-center ${order.status === 'completed' ? 'bg-gray-50' : 'bg-red-50 opacity-60'}`}>
                      <span>{order.drinkName} - {order.clinicName}</span>
                      <span className="text-xs text-gray-400">{order.status === 'completed' ? '✓' : '✗'}</span>
                    </div>
                  ))}
                </div>
              </div>
      ...
      {showOrderSuccess && (
        <div className="fixed bottom-24 left-1/2 -translate-x-1/2 bg-emerald-800 text-white px-8 py-4 rounded-2xl shadow-2xl z-[100] animate-bounce font-bold">
          تم إرسال طلبك بنجاح!
        </div>
      )}
