  ...
  return (
    <div className={`min-h-screen ${COLORS.bgLight} flex flex-col pb-24 font-sans`} dir="rtl">
      ...
      <main className="flex-1 max-w-7xl mx-auto w-full px-5 py-6">
        ...
        {view === 'cart' && (
          <div className="max-w-2xl mx-auto bg-white rounded-[2.5rem] shadow-xl overflow-hidden animate-fadeIn">
            <div className="p-6 bg-[#2D1B14] text-white flex items-center justify-between">
              <h2 className="text-xl font-black">سلة الطلبات</h2>
              <button onClick={() => setView('menu')} className="p-2 hover:bg-white/10 rounded-full"><Icons.ChevronRight /></button>
            </div>
            ...
            <div className="p-6 space-y-6">
              {Object.keys(cart).length === 0 ? (
                <div className="text-center py-10 opacity-50">
                  <Icons.ShoppingCart />
                  <p className="mt-4 font-bold">السلة فارغة</p>
                  <button onClick={() => setView('menu')} className="mt-4 text-amber-600 font-black text-sm">تصفح القائمة</button>
                </div>
              ) : (
                <>
                  <div className="space-y-4">
                    {Object.values(cart).map((item: CartItem) => (
                      <div key={item.drink.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-xl">
                        <div className="flex items-center gap-3">
                          <img src={item.drink.image} className="w-12 h-12 rounded-lg object-cover" alt={item.drink.name} />
                          <div>
                            <h4 className="font-bold text-sm text-[#2D1B14]">{item.drink.name}</h4>
                            <p className="text-xs text-gray-500">{item.drink.price} ج.م</p>
                          </div>
                        </div>
                        <div className="flex items-center gap-3 bg-white rounded-lg shadow-sm px-2 py-1">
                          <button onClick={() => updateCart(item.drink, -1)} className="p-1 text-amber-600"><Icons.Minus /></button>
                          <span className="font-black w-4 text-center text-sm">{item.quantity}</span>
                          <button onClick={() => updateCart(item.drink, 1)} className="p-1 text-amber-600"><Icons.Plus /></button>
                        </div>
                      </div>
                    ))}
                  </div>
                  ...
                  <div className="border-t border-dashed border-gray-200 pt-4">
                    <div className="flex justify-between items-center mb-6">
                      ...
                      <span className="font-black text-2xl text-[#2D1B14]">{cartTotalPrice} ج.م</span>
                    </div>

                    <div className="space-y-4 bg-amber-50/50 p-5 rounded-2xl border border-amber-100">
                      <h3 className="font-black text-sm text-amber-900 mb-2">بيانات التوصيل</h3>
                      
                      <div className="grid grid-cols-2 gap-3">
                        <div>
                          <label className="block text-[10px] font-bold text-gray-500 mb-1">الدور</label>
                          <select 
                            value={floorNumber} 
                            onChange={(e) => { setFloorNumber(e.target.value); setClinicNumber(""); }}
                            className="w-full p-2.5 rounded-xl border border-gray-200 bg-white text-sm font-bold focus:ring-2 focus:ring-amber-500 outline-none"
                          >
                            <option value="">اختر</option>
                            <option value="0">الأرضي (G)</option>
                            <option value="1">الأول (F)</option>
                            <option value="2">الثاني (S)</option>
                            <option value="3">الثالث (3)</option>
                          </select>
                        </div>
                        
                        <div>
                          <label className="block text-[10px] font-bold text-gray-500 mb-1">العيادة / الغرفة</label>
                          <select 
                            value={clinicNumber} 
                            onChange={(e) => setClinicNumber(e.target.value)}
                            disabled={!floorNumber}
                            className="w-full p-2.5 rounded-xl border border-gray-200 bg-white text-sm font-bold focus:ring-2 focus:ring-amber-500 outline-none disabled:opacity-50"
                          >
                            <option value="">اختر</option>
                      ...
                      <div>
                        <label className="block text-[10px] font-bold text-gray-500 mb-1">اسم الطالب / رقم التليفون</label>
                        <input 
                          type="text" 
                          value={contactInfo}
                          onChange={(e) => setContactInfo(e.target.value)}
                          placeholder="مثال: د. محمد - 010xxxx"
                          className="w-full p-2.5 rounded-xl border border-gray-200 bg-white text-sm font-bold focus:ring-2 focus:ring-amber-500 outline-none"
                        />
                      </div>

                      <div>
                        <label className="block text-[10px] font-bold text-gray-500 mb-1">ملاحظات إضافية (اختياري)</label>
                        <textarea 
                          value={orderNote}
                          onChange={(e) => setOrderNote(e.target.value)}
                          placeholder="سكر زيادة، بدون ثلج..."
                          className="w-full p-2.5 rounded-xl border border-gray-200 bg-white text-sm font-bold focus:ring-2 focus:ring-amber-500 outline-none h-20 resize-none"
                        />
                      </div>
                    </div>
                    ...
                    <div className="flex gap-3 mt-6">
                      <button 
                        ...
                        className="flex-1 bg-[#2D1B14] text-white py-3.5 rounded-xl font-black shadow-lg active:scale-95 transition-transform disabled:opacity-70 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                      >
                        {isPlacingOrder ? 'جاري الطلب...' : 'تأكيد الطلب'}
                        {!isPlacingOrder && <Icons.Check />}
                      </button>
                      <button 
                        onClick={cancelOrder}
                        className="px-4 py-3.5 rounded-xl font-bold text-red-500 bg-red-50 hover:bg-red-100 transition-colors"
                      >
                        <Icons.Trash />
                      </button>
                    </div>
        ...
        {view === 'admin' && (
          <div className="space-y-6 animate-fadeIn">
            <div className="flex items-center justify-between bg-white p-4 rounded-2xl shadow-sm">
              <h2 className="text-xl font-black text-[#2D1B14]">لوحة التحكم</h2>
              <div className="flex items-center gap-3">
                <span className="text-[10px] font-bold text-gray-400">آخر تحديث: {lastSyncTime}</span>
                <button onClick={() => setView('menu')} className="text-sm font-bold text-amber-600">خروج</button>
              </div>
            </div>
                ...
                orders.slice().reverse().map(order => (
                  <div key={order.id} className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100">
                    <div className="flex justify-between items-start mb-4">
                      <div>
                        <span className="inline-block px-2 py-1 bg-amber-100 text-amber-800 text-[10px] font-black rounded-md mb-1">#{order.id}</span>
                        <h3 className="font-bold text-[#2D1B14]">{order.contactInfo}</h3>
                        <p className="text-xs text-gray-500">الدور {order.floorNumber} - {order.clinicNumber}</p>
                      </div>
                      <span className="text-[10px] font-bold text-gray-400">{new Date(order.timestamp).toLocaleTimeString('ar-EG')}</span>
                    </div>
                    
                    <div className="space-y-2 mb-4">
                      {order.items.map((item, idx) => (
                        <div key={idx} className="flex justify-between text-sm">
                          <span className="text-gray-700">x{item.quantity} {item.drinkName}</span>
                    ...
                    <div className="border-t border-gray-100 pt-3 flex justify-between items-center">
                      <span className="font-black text-lg">{order.totalPrice} ج.م</span>
                      <span className={`px-3 py-1 rounded-full text-[10px] font-black ${order.status === 'completed' ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-600'}`}>
      ...
      {showSuccessModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-5 bg-black/60 backdrop-blur-sm animate-fadeIn">
          <div className="bg-white rounded-[2rem] p-8 max-w-sm w-full text-center shadow-2xl transform transition-all scale-100">
            <div className="w-20 h-20 bg-green-100 rounded-full flex items-center justify-center mx-auto mb-6 text-green-600">
              <Icons.Check />
            </div>
            <h3 className="text-2xl font-black text-[#2D1B14] mb-2">تم الطلب بنجاح!</h3>
            <p className="text-gray-500 mb-8 font-medium">سيصلك طلبك في أقرب وقت.</p>
            <button 
              onClick={() => { setShowSuccessModal(false); setView('menu'); }}
              className="w-full py-3.5 rounded-xl bg-[#2D1B14] text-white font-black shadow-lg active:scale-95 transition-transform"
            >
              حسناً
            </button>
          </div>
      ...
      {showAdminLogin && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-5 bg-black/60 backdrop-blur-sm animate-fadeIn">
          <div className="bg-white rounded-[2rem] p-8 max-w-sm w-full shadow-2xl">
            <h3 className="text-xl font-black text-[#2D1B14] mb-6 text-center">دخول الإدارة</h3>
            <input 
              type="password" 
              value={adminPassInput}
              onChange={(e) => setAdminPassInput(e.target.value)}
              placeholder="كلمة المرور"
              className="w-full p-3 rounded-xl border border-gray-200 bg-gray-50 text-center font-bold mb-4 focus:ring-2 focus:ring-amber-500 outline-none"
            />
            <div className="flex gap-3">
              <button 
                ...
                className="flex-1 py-3 rounded-xl bg-[#2D1B14] text-white font-black shadow-lg active:scale-95 transition-transform"
              >
                دخول
              </button>
              <button 
